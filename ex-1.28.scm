...
; This starts from an alternate form of Fermat's litter theorem, which states that if n is a prime number and a is any positive integer less than n, then a^(n-1) is congruent to 1 modulo n. 
...
; It is also possible to prove that if n is an odd number that is not prime, then, for at least half the numbers a < n, computing a^(n-1) in this way will reveal a nontrivial root of 1 modulo n. (This is why the Miller-Rabin test cannot be fooled.) 
